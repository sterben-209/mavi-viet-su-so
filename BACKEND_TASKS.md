...
- [x] **API Lấy danh sách bài viết:** Cần hỗ trợ phân trang (pagination), tìm kiếm và lọc theo `category`, `grade`.
- [x] **API Chi tiết bài viết:** Lấy bài viết theo `slug` (Frontend sử dụng slug cho URL).
...
- [x] **API Map Pins:** Tạo endpoint trả về danh sách `MapPin` (lat, lng, title, slug, ...).
- [x] **Lọc điểm trên bản đồ:** Hỗ trợ lọc theo `grade` và `period` (Giai đoạn lịch sử).
...
- [x] **API Stats:** Tạo endpoint trả về các con số thống kê cho trang chủ.
